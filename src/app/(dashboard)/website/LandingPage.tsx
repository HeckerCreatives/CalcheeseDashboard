...
export default function LandingPage() {
  ...
  return (
    <main className=' relative w-full max-w-[1920px] h-[105dvh] bg-white overflow-x-hidden overflow-y-hidden font-spenbeb text-amber-900'
    // style={{backgroundImage: "url('/assets/BG.png')" , backgroundPosition:'top right', backgroundRepeat:'no-repeat', backgroundSize:'contain'}}
    >
            <img src="/assets/BG.png" alt="tab" className=' h-full w-[80%] absolute right-0 translate-y-10' />

        <div className=' relative w-full'>
            <img src="/assets/Top TAB.png" alt="tab" className=' w-full absolute' />
            <img src="/assets/CalCheese World Logo.png" alt="logo" width={300} className=' relative z-50 top-10 left-10'/>

        </div>

        <div className=' w-full h-full grid grid-cols-[1fr_700px]'>

        <div className=' w-full h-full  '>
            <div className=' w-full h-full relative flex items-center justify-center'>

                <div className=' relative flex flex-col items-center justify-center'>
                    <div className=' absolute z-30 top-0 -translate-y-12 flex items-center justify-center'>
                        <img src="/assets/Title HOLDER.png" alt="title-holder" className=' ' loading='lazy' />
                        <h1 className=' absolute text-4xl font-bold uppercase text-white outlined-text'>{title}</h1>
                    </div>
                    <img src="/assets/Big TAB.png" alt="big-holder" className=' relative z-20' loading='lazy'/>
                    ...
                    <div className=' absolute z-30 left-0 mb-10 flex flex-col items-center justify-center gap-6'>
                        {tabs.map((item, index) => (
                        <button key={item.value} onClick={() => {setTab(item.value), setTitle(item.title)}}>
                            <img src={item.icon} alt="tab" width={60} className={`rounded-3xl cursor-pointer hover:scale-110 transition-all duration-300 ${tab === item.value && 'border-6 border-blue-500'} `} />
                        </button>
